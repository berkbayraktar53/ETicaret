using ETicaret.MvcWebUI.Filter;
using ETicaret.MvcWebUI.Models.i;
using System;
...
namespace ETicaret.MvcWebUI.Controllers
{
    public class iController : BaseController
    {
        ...
        {
            IQueryable<ETicaret.MvcWebUI.DB.Products> products = context.Products.OrderByDescending(x => x.AddedDate).Where(x => x.IsDeleted == false || x.IsDeleted == null);
            ETicaret.MvcWebUI.DB.Categories category = null;
            if (id > 0)
            {
        ...
        [HttpPost]
        [MyAuthorization]
        public ActionResult Product(ETicaret.MvcWebUI.DB.Comments comment)
        {
            try
            {
        ...
        public ActionResult Buy(string Address)
        {
            if (IsLogon())
            {
                try
                {
                    ...
                    var order = new ETicaret.MvcWebUI.DB.Orders()
                    {
                    ...
                    {
                        var oDetail = new ETicaret.MvcWebUI.DB.OrderDetails();
        ...
        public ActionResult Buy()
        {
            if (IsLogon())
            {
                ...
                IQueryable<ETicaret.MvcWebUI.DB.Orders> orders;
